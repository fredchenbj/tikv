...
impl<T: RaftStoreRouter + 'static, E: Engine> tikvpb_grpc::Tikv for Service<T, E> {
    ...
    fn raw_batch_get_by_index(
        ...
        let timer = GRPC_MSG_HISTOGRAM_VEC.raw_batch_get_by_index.start_coarse_timer();
    ...
    fn raw_batch_put(
        ...
        let command_duration = tikv_util::time::Instant::now_coarse();
        let table = if req.get_pairs().len() > 0 {
            let kv_pair = req.get_pairs().get(0).unwrap();
            keys::get_table_from_key(&kv_pair.get_key())
        } else {
            String::from("default")
        };
        debug!("metric table: {}", table);
        ...
        let future = future_raw_batch_put(&self.storage, req)
            ...
            .map(move |_| {
                let time_duration = command_duration.elapsed();
                TABLE_GRPC_COMMAND_DURATION_HISTOGRAM_VEC
                    .with_label_values(&["raw_batch_put", &table])
                    .observe(tikv_util::time::duration_to_sec(time_duration));
                timer.observe_duration()
            })
            .map_err(move |e| {
    ...
    fn raw_batch_update(
        ...
        let future = future
            ...
            .map(move |_| {
                let time_duration = command_duration.elapsed();
                TABLE_GRPC_COMMAND_DURATION_HISTOGRAM_VEC
                    .with_label_values(&["raw_batch_update", &table])
                    .observe(tikv_util::time::duration_to_sec(time_duration));
                timer.observe_duration()
            })
            .map_err(move |e| {
...
fn handle_batch_commands_request<E: Engine>(
    ...
    let timer3 = TABLE_GRPC_COMMAND_DURATION_HISTOGRAM_VEC
    ...
    match req.cmd {
        ...
        Some(BatchCommandsRequest_Request_oneof_cmd::RawGetByIndex(req)) => {
            ...
            let resp = future_raw_get_by_index(&storage, req)
                .map(oneof!(BatchCommandsResponse_Response_oneof_cmd::RawGetByIndex))
...
fn extract_kv_rows(res: storage::Result<Vec<storage::Result<storage::KvPair>>>, index_map:
